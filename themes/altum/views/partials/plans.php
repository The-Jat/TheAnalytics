...
<div class="pricing pricing-palden d-flex flex-wrap justify-content-center">
        ...
        <div class="pricing-item">
            <div class="pricing-deco" style="<?= settings()->plan_free->color ? 'background-color: ' . settings()->plan_free->color : null ?>">
                ...
                <svg class="pricing-deco-img" enable-background="new 0 0 300 100" height="100px" id="Layer_1" preserveAspectRatio="none" version="1.1" viewBox="0 0 300 100" width="300px" x="0px" xml:space="preserve" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg" y="0px">
                    ...
                    <path class="deco-layer deco-layer-2" d="M-34.667,62.998c0,0,56-45.667,120.316-27.839C167.484,57.842,197,41.332,232.286,30.428&#x000A;	c53.07-16.399,104.047,36.903,104.047,36.903l1.333,36.667l-372-2.954L-34.667,62.998z"></path>
                </svg>
            ...
            <a href="<?= url('register') ?>" class="btn btn-primary btn-lg rounded-pill pricing-action <?= is_logged_in() && $this->user->plan_id != 'free' ? 'disabled' : null ?>" style="<?= settings()->plan_free->color ? 'background-color: ' . settings()->plan_free->color : null ?>"><?= l('plans.choose') ?></a>
            <?php endif ?>
        </div>
        ...
        >
            <div class="pricing-deco" style="<?= $plan->color ? 'background-color: ' . $plan->color : null ?>">
                ...
                <div class="pricing-sub">
                    <span><?= $plan->translations->{\Altum\Language::$name}->description ?: $plan->description ?></span>
                </div>
            </div>
            ...
            <?php if(settings()->users->register_is_enabled || is_logged_in()): ?>
            <a href="<?= url('register?redirect=pay/' . $plan->plan_id) ?>" class="btn btn-primary btn-lg rounded-pill pricing-action <?= is_logged_in() && $this->user->plan_id == $plan->plan_id && (new \DateTime($this->user->plan_expiration_date)) > (new \DateTime())->modify('+10 years') ? 'disabled' : null ?>" style="<?= $plan->color ? 'background-color: ' . $plan->color : null ?>">
                <?php if(is_logged_in()): ?>
    ...
    <?php if(settings()->plan_custom->status == 1): ?>
        <div class="pricing-item">
            ...
            <?php if(settings()->users->register_is_enabled || is_logged_in()): ?>
            <a href="<?= settings()->plan_custom->custom_button_url ?>" class="btn btn-primary btn-lg rounded-pill pricing-action" style="<?= settings()->plan_custom->color ? 'background-color: ' . settings()->plan_custom->color : null ?>"><?= l('plans.contact') ?></a>
            <?php endif ?>
        </div>
    <?php endif ?>
