...
<div class="altum-animate altum-animate-fill-none altum-animate-fade-in">
    ...
    <?php else: ?>
                ...
                <div class="d-flex justify-content-between mb-1">
                    <div>
                        ...
                        <?php if($row->country_code && ($data->cities_link ?? true)): ?>
                            <a href="#" data-target="#cities_modal" data-toggle="modal" data-country-code="<?= $row->country_code ?>" class=""><?= $row->country_code ? get_country_from_country_code($row->country_code) : l('global.unknown') ?></a>
                        <?php else: ?>
        ...
        <?php if($data->total_rows > count($data->rows)): ?>
            <a href="<?= url($this->base_url_path . 'countries') ?>"><?= sprintf(l('global.view_x_more'), nr($data->total_rows - count($data->rows))) ?></a>
        <?php endif ?>
