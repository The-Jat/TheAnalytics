...
<div class="altum-animate altum-animate-fill-none altum-animate-fade-in">
    ...
    <?php else: ?>
            ...
            <div class="mb-3 row-fade-show-icon">
                <div class="d-flex justify-content-between mb-1">
                    <div class="text-truncate">
                        <?php if($row->utm): ?>
                            <a href="#" data-target="#utms_medium_campaign_modal" data-toggle="modal" data-utm-source="<?= $row->utm ?>" class=""><?= $row->utm ?></a>
                        <?php else: ?>
                            <?= l('global.none') ?>
                        <?php endif ?>
                    </div>
        ...
        <?php if($data->total_rows > count($data->rows)): ?>
            <a href="<?= url($this->base_url_path . 'utms') ?>"><?= sprintf(l('global.view_x_more'), nr($data->total_rows - count($data->rows))) ?></a>
        <?php endif ?>
