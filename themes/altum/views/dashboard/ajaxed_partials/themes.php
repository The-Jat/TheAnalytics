...
<div class="altum-animate altum-animate-fill-none altum-animate-fade-in">
    <div class="d-flex justify-content-between mb-2">
        <div>
            <small class="text-muted font-weight-bold text-uppercase"><?= l('dashboard.themes.theme') ?></small>
        </div>
    ...
    <?php else: ?>
        ...
        <?php foreach($data->rows as $row): ?>
            ...
            <div class="mb-3 row-fade-show-icon">
                <div class="d-flex justify-content-between mb-1">
                    <div>
                        <span><?= $row->theme ? l('dashboard.themes.' . $row->theme) : l('global.unknown') ?></span>
                    </div>
        ...
        <?php if($data->total_rows > count($data->rows)): ?>
            <a href="<?= url($this->base_url_path . 'screen-resolutions') ?>"><?= sprintf(l('global.view_x_more'), nr($data->total_rows - count($data->rows))) ?></a>
        <?php endif ?>
