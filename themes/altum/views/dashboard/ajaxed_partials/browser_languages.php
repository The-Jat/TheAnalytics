    ...
    <?php else: ?>
        ...
        <?php foreach($data->rows as $row): ?>
            ...
            <div class="mb-3 row-fade-show-icon">
                <div class="d-flex justify-content-between mb-1">
                    <div>
                        <span><?= $row->browser_language ? get_language_from_locale($row->browser_language) : l('global.unknown') ?></span>
                    </div>
        ...
        <?php if($data->total_rows > count($data->rows)): ?>
            <a href="<?= url($this->base_url_path . 'browser-languages') ?>"><?= sprintf(l('global.view_x_more'), nr($data->total_rows - count($data->rows))) ?></a>
        <?php endif ?>
