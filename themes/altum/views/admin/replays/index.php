...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0 text-truncate"><i class="fas fa-fw fa-xs fa-video text-primary-900 mr-2"></i> <?= l('admin_replays.header') ?></h1>

    <div class="d-flex position-relative d-print-none">
        <div class="ml-3">
            <a href="<?= url('admin/statistics/sessions_replays') ?>" class="btn btn-gray-300" data-tooltip title="<?= l('global.statistics') ?>">
        ...
        <div class="ml-3">
            <div class="dropdown">
                <button type="button" class="btn btn-gray-300 dropdown-toggle-simple <?= count($data->replays) ? null : 'disabled' ?>" data-toggle="dropdown" data-boundary="viewport" data-tooltip title="<?= l('global.export') ?>" data-tooltip-hide-on-click>
                    <i class="fas fa-fw fa-sm fa-download"></i>
                </button>

                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/replays?' . $data->filters->get_get() . '&export=csv')  ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->csv ? null : get_plan_feature_disabled_info() ?>>
                        <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('admin/replays?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->json ? null : get_plan_feature_disabled_info() ?>>
                ...
                <div class="dropdown-menu dropdown-menu-right filters-dropdown">
                    ...
                    <form action="" method="get" role="form">
                        <div class="form-group px-4">
                            <label for="is_offloaded" class="small"><?= l('replay.is_offloaded') ?></label>
                            <select name="is_offloaded" id="is_offloaded" class="custom-select custom-select-sm">
                                <option value=""><?= l('global.all') ?></option>
                                <option value="1" <?= isset($data->filters->filters['is_offloaded']) && $data->filters->filters['is_offloaded'] == '1' ? 'selected="selected"' : null ?>><?= l('global.yes') ?></option>
                                <option value="0" <?= isset($data->filters->filters['is_offloaded']) && $data->filters->filters['is_offloaded'] == '0' ? 'selected="selected"' : null ?>><?= l('global.no') ?></option>
                            </select>
                        </div>
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="order_by" id="filters_order_by" class="custom-select custom-select-sm">
                                <option value="replay_id" <?= $data->filters->order_by == 'replay_id' ? 'selected="selected"' : null ?>><?= l('global.id') ?></option>
                                <option value="datetime" <?= $data->filters->order_by == 'datetime' ? 'selected="selected"' : null ?>><?= l('global.filters.order_by_datetime') ?></option>
                                <option value="last_datetime" <?= $data->filters->order_by == 'last_datetime' ? 'selected="selected"' : null ?>><?= l('global.filters.order_by_last_datetime') ?></option>
                                <option value="expiration_date" <?= $data->filters->order_by == 'expiration_date' ? 'selected="selected"' : null ?>><?= l('replay.expiration_date') ?></option>
                                <option value="events" <?= $data->filters->order_by == 'events' ? 'selected="selected"' : null ?>><?= l('replay.events') ?></option>
                                <option value="size" <?= $data->filters->order_by == 'size' ? 'selected="selected"' : null ?>><?= l('replay.size') ?></option>
                            </select>
                        </div>
...
<form id="table" action="<?= SITE_URL . 'admin/replays/bulk' ?>" method="post" role="form">
                ...
                <tr>
                    ...
                    <td>
                        <div class="d-flex justify-content-end">
                            <?= include_view(THEME_PATH . 'views/admin/replays/admin_replay_dropdown_button.php', ['id' => $row->replay_id]) ?>
                        </div>
                    </td>
                </tr>
            <?php endforeach ?>
